...
document.addEventListener('DOMContentLoaded', function () {
  ...
  pinAdd.addEventListener('click', () => {
    ...
    const country = pinCountry.value, year = pinYear.value;
    if (pins.some(p => p.country === country && p.year === year)) return;
    pins.push({ country, year });
    renderChips();
    renderAll();
  });
  ...
  function renderAll() {
    ...
    // ASFR
    const asfrDatasets = pinData.map((p, i) => ({ data: toPoints(p.d.asfr_ages, p.asfrVals), borderColor: PIN_COLORS[i], backgroundColor: 'transparent', fill: false, tension: 0.3, pointRadius: 0, borderWidth: 2 }));
    const asfrLegend = pinData.map((p, i) => ({ color: PIN_COLORS[i], text: `${p.country}, ${p.year}` }));
    ...
    const pxDatasets = pinData.map((p, i) => ({ data: toPoints(p.d.px_ages, survivorship(p.d.px_ages, p.pxVals)), borderColor: PIN_COLORS[i], backgroundColor: 'transparent', fill: false, tension: 0.3, pointRadius: 0, borderWidth: 2 }));
    const pxLegend = pinData.map((p, i) => ({ color: PIN_COLORS[i], text: `${p.country}, ${p.year}` }));
    ...
    const growthDatasets = pinData.map((p, i) => {
      ...
      const initialDistFull = resample(p.d.pop_ages, p.popVals);
      const pxFull = expandPxStepwise(p.d.px_ages, p.pxVals);
      const rates = projectGrowthRate(p.d.asfr_ages, p.asfrVals, pxFull, initialDistFull, years);
      return { data: rates.map((rate, t) => ({ x: baseYear + t, y: +(rate * 100).toFixed(3) })), borderColor: PIN_COLORS[i], backgroundColor: 'transparent', fill: false, tension: 0.2, pointRadius: 0, borderWidth: 2, r: stableDist(p.d.asfr_ages, p.asfrVals, p.d.px_ages, p.pxVals).r };
    });
    const growthLegend = pinData.map((p, i) => ({ color: PIN_COLORS[i], text: `${p.country}, ${p.year} — converges to r = ${(growthDatasets[i].r * 100).toFixed(2)}%` }));
